...
下周计划分为用户态实验和内核实验两条线推进。

用户态实验方面，我计划继续完善 Priority Executor 的 trace 可视化，将任务状态变化、优先级变化和 aging promote 过程整理成更直观的 Mermaid 图或时间线。同时，可以补充不同调度策略的对比实验，例如 FIFO、严格优先级、优先级加 aging、round-robin 等，进一步分析它们在公平性和响应性上的差异。

内核实验方面，我暂时不直接修改 ArceOS / StarryOS 的全局调度器，而是优先在 I/O 路径中做局部插桩。重点观察 syscall read/write、VFS、block cache、virtio-blk 和 WaitQueue 路径，记录不同进程的等待时长和唤醒次数。如果确认存在明显不公平，再尝试在局部 WaitQueue 或 block I/O 请求队列中引入 priority-aware notify 和 aging 策略，并重跑 iozone 多进程测试，观察 `Min throughput per process` 是否改善。
...
```text
100-LoC-stack-less-coroutine-执行流动态跟踪分析.md
200行-无栈协程-执行流动态跟踪分析.md
green-thread-执行流动态跟踪.md
priority-executor-design.md
execution-flow-comparison.md
week-report.md
```
...
第一，我把你原来“学习摘要”中的内容展开成了完整叙述，把 Tokio Future、100 行无栈协程、200 行 Future executor 和绿色线程放在同一条执行流主线上比较，而不是只列结论。这样周报更像一份完整学习报告。

第二，我补全了动态跟踪部分，把 trace 点从简单的 `spawn / poll / pending / wake / complete` 展开为不同实验中的具体跟踪对象，例如 Executor 队列、Reactor 注册、Waker 唤醒、Parker park/unpark、绿色线程 `switch` 等。这样可以体现你确实做了动态跟踪，而不是只读了材料。

第三，我把优先级 Future Executor 的实践部分写完整了，包括为什么选择 Future Executor、三队列 ready queue、priority-aware Waker、aging 机制和测试结果。这对应导师任务中的“选择一种执行流机制扩展优先级支持，并写测例验证正确性”。

第四，我把 ArceOS / StarryOS 比赛任务自然接到了周报后半部分，重点说明用户态实验如何启发 WaitQueue / block I/O 公平性分析，同时避免夸大结果。现在的表述是“提供分析方向和局部 PoC 思路”，而不是直接声称已经解决 iozone 问题，这样更严谨。